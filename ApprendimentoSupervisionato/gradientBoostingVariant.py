...
for tree in max_tree_range:    
    gradBoost = GradientBoostingClassifier(n_estimators=tree, random_state = 0)
    gradBoost.fit(X_train, Y_train)
    Y_pred_train = gradBoost.predict(X_train)
    Y_pred_test = gradBoost.predict(X_test)
    ...
    importances.append(pd.DataFrame({'importance':np.round(gradBoost.feature_importances_,5)}, index=X_col.columns))
